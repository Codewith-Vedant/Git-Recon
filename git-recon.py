...
'''
    ...
    html_content += f'''
        <div class="footer">
            <p>Generated by GitRecon v2.0 | Scan completed at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
        </div>
    </div>
</body>
</html>'''
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GitRecon v2.0 - Advanced GitHub Secret Scanner (Enhanced v1.5 compatible)",
...
Tips:
  ...
  - Consider using --max-pages 2 for faster scans
    ...
    parser.add_argument("-c", "--category", default="ALL", help="Category to scan (ALL, AWS, GITHUB, etc.) - v2.0 scans all by default")
    ...
    print_info(f"Category: {args.category} (v2.0 scans all patterns)")
